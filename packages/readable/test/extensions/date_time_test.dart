...
void main() {
  ...
  test('sameDayNextWeek', () {
    // Friday, April 15th, 2022, 9:30 AM
    final date = DateTime(2022, 4, 15, 9, 30);
    final sameDayNextWeek = date.sameDayNextWeek;
    expect(sameDayNextWeek.year, equals(2022));
    expect(sameDayNextWeek.month, equals(4));
    expect(sameDayNextWeek.day, equals(22)); // Same day next week
    expect(sameDayNextWeek.hour, equals(9));
    expect(sameDayNextWeek.minute, equals(30));
    expect(sameDayNextWeek.second, equals(0));
  });
  test('sameDayPreviousWeek', () {
    final date = DateTime(2023, 4, 15, 12);
    final sameDayPreviousWeek = date.sameDayPreviousWeek;
    expect(sameDayPreviousWeek.year, equals(2023));
    expect(sameDayPreviousWeek.month, equals(4));
    expect(sameDayPreviousWeek.day, equals(8));
    expect(sameDayPreviousWeek.hour, equals(12));
  });
  test(
      'isLeapYear should return true for leap years and false for non-leap years',
      () {
    // Test a leap year
  ...
  test('dateOnly returns DateTime with no time', () {
    // Arrange
    // 2022-04-15 16:30:00.000
    final dt = DateTime(2022, 4, 15, 16, 30);

    // Act
    final dateOnly = dt.dateOnly;

    // Assert
    expect(dateOnly.year, equals(2022));
    expect(dateOnly.month, equals(4));
    expect(dateOnly.day, equals(15));
    expect(dateOnly.hour, equals(0));
    expect(dateOnly.minute, equals(0));
    expect(dateOnly.second, equals(0));
    expect(dateOnly.millisecond, equals(0));
  });
